...
from sas_rec.dataset import data_partition_ratings, SasRecModSequence
from sas_rec.models.reg_model import SASRecRegModel
...
class Experiment(App):
    ...
    def load_dataset(self):
        ...
        self.dataset = data_partition_ratings(fname, seed=self.args.seed,
        ...
        train_sequence = SasRecModSequence(
            user_train, self.uir,
        ...
        new_user_valid = {}
        for user, val_seq in user_valid.items():
            new_val_seq = user_train[user] + val_seq
            new_user_valid[user] = new_val_seq

        validation_sequence = SasRecModSequence(
        ...
        for user, test_seq in user_test.items():
            new_test_seq = user_train[user] + user_valid[user] + test_seq
            new_user_test[user] = new_test_seq

        test_sequence = SasRecModSequence(
            users=new_user_test,
            uir_map=self.uir,
            num_users=self.user_num,
            num_items=self.item_num,
    ...
    def evaluate_model(self, test_sequence):
        ...
        """
        mse, mae, acc = self.model.evaluate(test_sequence)
        outs = {
            'test_mse': mse,
            'test_mae': mae,
            'test_acc': acc
    ...
    def run(self):
        ...
        metrics["val_mae"] = np.round(history.history['val_mae'][-1], 4)
